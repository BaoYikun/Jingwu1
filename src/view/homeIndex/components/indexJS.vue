  ...
  <div class="customBox container">
    ...
    <div class="custom-content w f-l">
      <div class="custom-list-box">
        <div class="polaroid f-l" v-for="(item, index) in newTeacher" :key="index">
          <img
            class="polaroid-img"
            :src="item.iconPath"
            alt="Norway"
          ...
          <div class="w">
            ...
            <div class="c-l-b">
              <span class="c-l-time">时长：{{ item.seconds }}分钟</span>
      ...
      <div class="hot-box">
        <p class="hot-title">近一个月最热教师</p>
        <ul class="hot-list-box">
          <li class="hot-list" v-for="(item, index) in hotTeacher" :key="index">
            <i class="h-l-icon"></i>
            ...
            <div class="hot-num-box">
              ...
              <span class="h-l-num">{{ item.playcount }}</span>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    loadData() {
      this.$get(this.newTeacherInfo.url, this.newTeacherInfo).then((res) => {
        console.log(res);
        if (res.data.dataList.length > 8) {
          for (var i = 0; i < 8; i++) {
            this.newTeacher.push(res.data.dataList[i]);
          }
        } else {
          this.newTeacher = res.data.dataList;
        }
      });
      this.$get(this.hotTeacherInfo.url, this.hotTeacherInfo).then((res) => {
        console.log(res);
        if (res.data.dataList.length > 6) {
          for (var i = 0; i < 6; i++) {
            this.hotTeacher.push(res.data.dataList[i]);
          }
        } else {
          this.hotTeacher = res.data.dataList;
        }
      });
    },
    pageJump() {
      this.$router.push({path:this.newTeacherInfo.jumpPage})
    },
    getImgUrl() {
      let url = '';
