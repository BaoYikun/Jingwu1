...
export default {
  ...
  methods: {
    ...
    search() {
      this.$post("cms-web/review/video/like/select/api?curPage=" + this.currentPage + "&pageSize=" + this.pageSize, {leibie: "gongzuofang",searchParams: this.searchInfo}).then((res) => {
        this.gongzuofangList = res.data.dataList;
        this.total = parseInt(res.data.paging.total);
        this.currentPage = parseInt(res.data.paging.curPage);
      });
